...
为什么组合逻辑会形成latch因为没有覆盖的条件导致寄存器enable拉低自然锁存保持原值。  
时序逻辑（触发器，Flip-flop）不是“不受影响”，而是“天生自带锁存功能”，所以它不需要（也不应该）用组合逻辑那种“覆盖全部条件”的方式来避免Latch。  
## 关于wire和reg
|组合逻辑|时序逻辑|
|---|---|
|no edge|pos/neg edge|
|no D触发器|has D触发器|
|output wire|output reg|
|立马输出|下一个上升/下降沿输出|

**组合逻辑用reg也可以，但是会延迟一个周期输出，这里输出就是接了一个D触发器**
